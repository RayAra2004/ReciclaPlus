# TRABALHO DE PI:  Título do Trabalho
Trabalho desenvolvido durante a disciplina de Banco de Dados do Integrado

# Sumário

### 1. COMPONENTES<br>
Integrantes do grupo<br>
primeiro_componente_do_grupo: cesbarbosa2005@gmail.com<br>
segundo_componente_do_grupo: dara.botecchia22@gmail.com<br>
terceiro_componente_do_grupo: raynan.araujo.ifes@gmail.com<br>
quarto_componente_do_grupo: kaua271203@gmail.com<br>

### 2.MINIMUNDO<br>
Descrever o mini-mundo! (Não deve ser maior do que 30 linhas, se necessário resumir para justar)
Entrevista com o usuário e identificação dos requisitos.(quando for o caso de sistemas com cliente real)
Descrição textual das regras de negócio definidas como um subconjunto do mundo real cujos elementos são propriedades que desejamos incluir, processar, armazenar, gerenciar, atualizar, e que descrevem a proposta/solução a ser desenvolvida.
<br>

> Várias pessoas nos dias de hoje não fazem o descarte de seus materiais da maneira correta,
muitas delas também não são orientadas de maneira consciente e objetiva de levar seu lixo ao
local certo.
Segundo o CGIRSVJ (Consórcio de Gestão Integrada de Resíduos Sólidos - Vale do Jaguaribe),
40% do lixo produzido no Brasil em 2019 foi descartado incorretamente. Das mais de 72,7
milhões de toneladas de lixo coletadas, cerca de 29,5 milhões foram descartadas
incorretamente, ou seja, em aterro controlado ou lixão, que são prejudiciais ao meio ambiente.
Somente 3% de 2 milhões de toneladas de resíduos eletrônicos descartados no Brasil foram
reciclados em 2019.
Tomando conhecimento desta situação, decidimos criar um sistema web/app no qual fornecerá
ao usuário os pontos de coletas próximos a ele, além de informá-lo sobre o correto descarte de
seus materiais.
 
 
### 3.PMC<br>
![image](https://github.com/RayAra2004/ReciclaPlus/assets/101998757/66fadb5b-da84-4852-bd50-3f36323416eb)



a) inclusão do PMC desenvolvido pelo grupo <br>

#### 3.1. EAP - Estrutura Analítica do Projeto
a)![image](https://github.com/RayAra2004/ReciclaPlus/assets/101998757/eacc1511-85ba-4e11-b14b-31bd40d88569)
b) ![image](https://github.com/RayAra2004/ReciclaPlus/assets/101998757/40287d4f-83e3-4eba-9ad0-13bd98c40d19)
![image](https://github.com/RayAra2004/ReciclaPlus/assets/101998757/fb733000-6473-4e48-94ce-35ffaeb55555)


#### 3.2. Requisitos funcionais e não funcionais
Incluir informações de: Identificador , Descrição e Prioridade
Exemplos:<br>
![image](https://github.com/RayAra2004/ReciclaPlus/assets/101998757/44b61883-9ffb-4d03-b388-7462c2a2b566)
![image](https://github.com/RayAra2004/ReciclaPlus/assets/101998757/60e81b49-863d-47df-aca3-e2d34fa30923)
![image](https://github.com/RayAra2004/ReciclaPlus/assets/101998757/ba5aedb5-9413-465f-9189-8b56b1cc72d2)
![image](https://github.com/RayAra2004/ReciclaPlus/assets/101998757/1a568304-0601-4536-bcbf-b2def4d1ce03)

#### 3.3 Validação da Ideia.
a) [Link do formulário desenvolvido](https://drive.google.com/file/d/1jUiEKDf0761iWvIaIyuKtdCvYRHQjsuE/view?usp=sharing)
b) [Link para Relatório/Apresentação de resultados obtidos
](https://drive.google.com/file/d/1jUiEKDf0761iWvIaIyuKtdCvYRHQjsuE/view?usp=sharing)
### 4.Personas e Histórias de usuário<br>
<img src="https://neilpatel.com/wp-content/uploads/2019/07/exemplo-carlos.png" Personas src="https://neilpatel.com/wp-content/uploads/2019/07/exemplo-carlos.png" width="500" height="500" /> <br>
a) inclusão dos Persons desenvolvidos pelo grupo<br>
<br>
<img src="https://miro.medium.com/max/1400/1*r5GVnOvqpMdxnGUYNRXqbg.png" UserStory src="https://miro.medium.com/max/1400/1*r5GVnOvqpMdxnGUYNRXqbg.png" width="500" height="300" /> <br>
b) inclusão das Histórias de usuário desenvolvidas pelo grupo
<br>


### 5. PROTÓTIPOS DO SISTEMA<br>
Neste ponto a codificação não e necessária, somente as ideias de telas devem ser desenvolvidas. O princípio aqui é pensar na criação da interface para identificar possíveis informações a serem armazenadas e/ou descartadas <br>

![image](https://github.com/RayAra2004/ReciclaPlus/assets/101998757/edada935-f569-497c-b3f0-97237615d24e)

#### 5.1 PROTÓTIPO DO SISTEMA MOBILE

link: https://quant-ux.com/#/test.html?h=a2aa10aBPyxN6ylcMAPhkFw6lyq5yhZu8xV0oreWDnXPHSN8EwX1w2Dl8Dti&ln=en

#### 5.2 PROTÓTIPO DO SISTEMA WEB

link: https://quant-ux.com/#/test.html?h=a2aa10acVZG0YoT3CXzyCFh4gpuaOy1mZ3QS6ED6IolQmMViBgNVGvr29CUa&ln=en
Link para o sistema web: http://reciclamais.great-site.net/index.html?i=1

#### 5.3 QUAIS PERGUNTAS PODEM SER RESPONDIDAS COM OS SISTEMA WEB/MOBILE PROPOSTOS?
    a) O sistema proposto poderá fornecer quais tipos de relatórios e informaçes? 
    b) Crie uma lista com os 5 principais relatórios que poderão ser obtidos por meio do sistema proposto!
    
> A Empresa DevCom precisa inicialmente dos seguintes relatórios:
* Relatório que informe quais são os gerentes de cada departamento incluindo as seguintes informações: número do departamento,  nome do departamento, e nome do gerente.
* Relatório de empregados por projeto incluindo as seguintes informações: número do projeto, nome do projeto, rg do empregado, nome do empregado e quantidade de horas de trabalho do empregado alocadas ao projeto.
* Relatório de empregados com dependentes incluindo as seguintes informações: rg do empregado, nome do empregado, nome do dependente, tipo de relação, data de nascimento do dependente e sexo do dependente.
* Relatório com a quantidade de empregados por cada departamento incluindo as seguintes informações: nome do departamento, supervisor e quantidade de empregados alocados no departamento.
* Relatório de supervisores e supervisionados incluindo as seguintes informações: nome do supervisor e nome do supervisionado.
 
 ### 6.MODELO CONCEITUAL<br>
    A) Utilizar a Notação adequada (Preferencialmente utilizar o BR Modelo 3)
    B) O mínimo de entidades do modelo conceitual pare este trabalho será igual a 4.
        * informe quais são as 3 principais entidades do sistema em densenvolvimento
      (se houverem mais de 3 entidades, pense na importância da entidade para o sistema)       
    C) Principais fluxos de informação/entidades do sistema (mínimo 2). <br>Dica: normalmente estes fluxos estão associados as tabelas que conterão maior quantidade de dados 
    D) Qualidade e Clareza
        Garantir que a semântica dos atributos seja clara no esquema (nomes coerentes com os dados).
        Criar o esquema de forma a garantir a redução de informação redundante, possibilidade de valores null, 
        e tuplas falsas (Aplicar os conceitos de normalização abordados).   
        
![image](https://github.com/RayAra2004/ReciclaPlus/assets/101998757/45ffa69e-3d3e-4985-b5e1-417785165c49)
      
    
#### 7 Descrição dos dados 
    [objeto]: [descrição do objeto]
    
    EXEMPLO:
    CLIENTE: Tabela que armazena as informações relativas ao cliente<br>
    CPF: campo que armazena o número de Cadastro de Pessoa Física para cada cliente da empresa.<br>

### 8	RASTREABILIDADE DOS ARTEFATOS<br>
        a) Historia de usuários vs protótipo (Histórias de Usuário e em qual tela do protótipo aquela HU está sendo realizada).
        b) Protótipo vs Modelo conceitual (Histórias de Usuário e em quais tabelas aquele dado está sendo registrado).
        (modelos devem obrigatoriamente estar em conformidade de rastreabilidade)

### 9	MODELO LÓGICO<br>
        ![image](https://github.com/RayAra2004/ReciclaPlus/assets/101998757/0bbc2f1c-c4d6-47d6-a639-f0addc332648)

### 10	MODELO FÍSICO<br>
        a) inclusão das instruções de criacão das estruturas em SQL/DDL 
        /* modelo_logico_16_05: */

CREATE TABLE USUARIO (
    Login varchar(90),
    Senha varchar(30),
    ID serial PRIMARY KEY,
    Nome varchar(50),
    Telefone bigint,
    fk_TIPO_USUARIO_ID int
);

CREATE TABLE INSTITUICAO (
    CNPJ varchar(18),
    Logo varchar(400),
    Telefone bigint,
    FK_USUARIO_ID int PRIMARY KEY,
    FK_ENDERECO_ID int
);

CREATE TABLE PESSOA (
    CPF varchar(14),
    DataNascimento date,
    FK_USUARIO_ID int PRIMARY KEY
);

CREATE TABLE PONTO_COLETA (
    Nome varchar(30),
    ID serial PRIMARY KEY,
    Imagem varchar(400),
    Nota float,
    FK_INSTITUICAO_FK_USUARIO_ID int,
    FK_ENDERECO_ID int
);

CREATE TABLE MATERIAIS_RECICLADOS (
    id serial PRIMARY KEY,
    descricao varchar(20)
);

CREATE TABLE TIPO_USUARIO (
    id serial PRIMARY KEY,
    descricao varchar(20)
);

CREATE TABLE ENDERECO (
    CEP varchar(20),
    FK_Tipo_logradouro_Tipo_logradouro_PK int,
    Logradouro varchar(50),
    FK_Estado_Estado_PK int,
    FK_Cidade_Cidade_PK int,
    FK_Bairro_Bairro_PK int,
    Numero int,
    ID serial PRIMARY KEY,
    Longitude bigint,
    Latitude bigint
);

CREATE TABLE AVALIACAO (
    id serial PRIMARY KEY,
    descricao varchar(140)
);

CREATE TABLE _Cadastra (
    fk_USUARIO_ID int,
    fk_PONTO_COLETA_ID int,
    qtd_validacao int,
    data timestamp
);

CREATE TABLE Tipo_logradouro (
    Tipo_logradouro_PK serial NOT NULL PRIMARY KEY,
    Tipo_logradouro varchar(30)
);

CREATE TABLE Estado (
    Estado_PK serial NOT NULL PRIMARY KEY,
    Estado varchar(2)
);

CREATE TABLE Cidade (
    Cidade_PK serial NOT NULL PRIMARY KEY,
    Cidade varchar(30)
);

CREATE TABLE Bairro (
    Bairro_PK serial NOT NULL PRIMARY KEY,
    Bairro varchar(30)
);

CREATE TABLE Leva (
    fk_MATERIAIS_RECICLADOS_id int,
    fk_PESSOA_FK_USUARIO_ID int
);

CREATE TABLE Recicla (
    fk_MATERIAIS_RECICLADOS_id int,
    fk_PONTO_COLETA_ID int
);

CREATE TABLE Envia_PESSOA_MATERIAIS_RECICLADOS_PONTO_COLETA (
    fk_PESSOA_FK_USUARIO_ID int,
    fk_MATERIAIS_RECICLADOS_id int,
    fk_PONTO_COLETA_ID int
);

CREATE TABLE Realiza_PESSOA_AVALIACAO_PONTO_COLETA (
    fk_PESSOA_FK_USUARIO_ID int,
    fk_AVALIACAO_id int,
    fk_PONTO_COLETA_ID int
);

CREATE TABLE Valida (
    fk_USUARIO_ID int,
    data timestamp,
    resposta int
);
 
ALTER TABLE USUARIO ADD CONSTRAINT FK_USUARIO_2
    FOREIGN KEY (fk_TIPO_USUARIO_ID)
    REFERENCES TIPO_USUARIO (id);
 
ALTER TABLE INSTITUICAO ADD CONSTRAINT FK_INSTITUICAO_2
    FOREIGN KEY (FK_USUARIO_ID)
    REFERENCES USUARIO (ID)
    ON DELETE CASCADE;
 
ALTER TABLE INSTITUICAO ADD CONSTRAINT FK_INSTITUICAO_3
    FOREIGN KEY (FK_ENDERECO_ID)
    REFERENCES ENDERECO (ID)
    ON DELETE CASCADE;
 
ALTER TABLE PESSOA ADD CONSTRAINT FK_PESSOA_2
    FOREIGN KEY (FK_USUARIO_ID)
    REFERENCES USUARIO (ID)
    ON DELETE CASCADE;
 
ALTER TABLE PONTO_COLETA ADD CONSTRAINT FK_PONTO_COLETA_2
    FOREIGN KEY (FK_INSTITUICAO_FK_USUARIO_ID)
    REFERENCES INSTITUICAO (FK_USUARIO_ID)
    ON DELETE CASCADE;
 
ALTER TABLE PONTO_COLETA ADD CONSTRAINT FK_PONTO_COLETA_3
    FOREIGN KEY (FK_ENDERECO_ID)
    REFERENCES ENDERECO (ID)
    ON DELETE CASCADE;

ALTER TABLE USUARIO ADD CONSTRAINT FK_TIPO_USUARIO_2
    FOREIGN KEY (fk_tipo_usuario_id)
    REFERENCES tipo_usuario (ID)
    ON DELETE RESTRICT;
 
ALTER TABLE ENDERECO ADD CONSTRAINT FK_ENDERECO_2
    FOREIGN KEY (FK_Tipo_logradouro_Tipo_logradouro_PK)
    REFERENCES Tipo_logradouro (Tipo_logradouro_PK)
    ON DELETE NO ACTION;
 
ALTER TABLE ENDERECO ADD CONSTRAINT FK_ENDERECO_3
    FOREIGN KEY (FK_Estado_Estado_PK)
    REFERENCES Estado (Estado_PK)
    ON DELETE NO ACTION;
 
ALTER TABLE ENDERECO ADD CONSTRAINT FK_ENDERECO_4
    FOREIGN KEY (FK_Cidade_Cidade_PK)
    REFERENCES Cidade (Cidade_PK)
    ON DELETE NO ACTION;
 
ALTER TABLE ENDERECO ADD CONSTRAINT FK_ENDERECO_5
    FOREIGN KEY (FK_Bairro_Bairro_PK)
    REFERENCES Bairro (Bairro_PK)
    ON DELETE NO ACTION;
 
ALTER TABLE _Cadastra ADD CONSTRAINT FK__Cadastra_1
    FOREIGN KEY (fk_USUARIO_ID)
    REFERENCES USUARIO (ID);
 
ALTER TABLE _Cadastra ADD CONSTRAINT FK__Cadastra_2
    FOREIGN KEY (fk_PONTO_COLETA_ID)
    REFERENCES PONTO_COLETA (ID);
 
ALTER TABLE Leva ADD CONSTRAINT FK_Leva_1
    FOREIGN KEY (fk_MATERIAIS_RECICLADOS_id)
    REFERENCES MATERIAIS_RECICLADOS (id)
    ON DELETE SET NULL;
 
ALTER TABLE Leva ADD CONSTRAINT FK_Leva_2
    FOREIGN KEY (fk_PESSOA_FK_USUARIO_ID)
    REFERENCES PESSOA (FK_USUARIO_ID)
    ON DELETE SET NULL;
 
ALTER TABLE Recicla ADD CONSTRAINT FK_Recicla_1
    FOREIGN KEY (fk_MATERIAIS_RECICLADOS_id)
    REFERENCES MATERIAIS_RECICLADOS (id)
    ON DELETE SET NULL;
 
ALTER TABLE Recicla ADD CONSTRAINT FK_Recicla_2
    FOREIGN KEY (fk_PONTO_COLETA_ID)
    REFERENCES PONTO_COLETA (ID)
    ON DELETE SET NULL;
 
ALTER TABLE Envia_PESSOA_MATERIAIS_RECICLADOS_PONTO_COLETA ADD CONSTRAINT FK_Envia_PESSOA_MATERIAIS_RECICLADOS_PONTO_COLETA_1
    FOREIGN KEY (fk_PESSOA_FK_USUARIO_ID)
    REFERENCES PESSOA (FK_USUARIO_ID)
    ON DELETE NO ACTION;
 
ALTER TABLE Envia_PESSOA_MATERIAIS_RECICLADOS_PONTO_COLETA ADD CONSTRAINT FK_Envia_PESSOA_MATERIAIS_RECICLADOS_PONTO_COLETA_2
    FOREIGN KEY (fk_MATERIAIS_RECICLADOS_id)
    REFERENCES MATERIAIS_RECICLADOS (id)
    ON DELETE NO ACTION;
 
ALTER TABLE Envia_PESSOA_MATERIAIS_RECICLADOS_PONTO_COLETA ADD CONSTRAINT FK_Envia_PESSOA_MATERIAIS_RECICLADOS_PONTO_COLETA_3
    FOREIGN KEY (fk_PONTO_COLETA_ID)
    REFERENCES PONTO_COLETA (ID)
    ON DELETE NO ACTION;
 
ALTER TABLE Realiza_PESSOA_AVALIACAO_PONTO_COLETA ADD CONSTRAINT FK_Realiza_PESSOA_AVALIACAO_PONTO_COLETA_1
    FOREIGN KEY (fk_PESSOA_FK_USUARIO_ID)
    REFERENCES PESSOA (FK_USUARIO_ID)
    ON DELETE NO ACTION;
 
ALTER TABLE Realiza_PESSOA_AVALIACAO_PONTO_COLETA ADD CONSTRAINT FK_Realiza_PESSOA_AVALIACAO_PONTO_COLETA_2
    FOREIGN KEY (fk_AVALIACAO_id)
    REFERENCES AVALIACAO (id)
    ON DELETE NO ACTION;
 
ALTER TABLE Realiza_PESSOA_AVALIACAO_PONTO_COLETA ADD CONSTRAINT FK_Realiza_PESSOA_AVALIACAO_PONTO_COLETA_3
    FOREIGN KEY (fk_PONTO_COLETA_ID)
    REFERENCES PONTO_COLETA (ID)
    ON DELETE NO ACTION;
 
ALTER TABLE Valida ADD CONSTRAINT FK_Valida_1
    FOREIGN KEY (fk_USUARIO_ID)
    REFERENCES USUARIO (ID)
    ON DELETE RESTRICT;

 
        
       
### 11	INSERT APLICADO NAS TABELAS DO BANCO DE DADOS<br>
        a) inclusão das instruções de inserção dos dados nas tabelas criadas pelo script de modelo físico
        -- Preenchendo a tabela Estado
INSERT INTO Estado (Estado_PK, Estado) VALUES (1, 'ES');

-- Preenchendo a tabela Tipo_logradouro
INSERT INTO Tipo_logradouro (Tipo_logradouro_PK, Tipo_logradouro) VALUES
(1, 'Rua'),
(2, 'Avenida'),
(3, 'Travessa'),
(4, 'Alameda');

-- Preenchendo a tabela Cidade
INSERT INTO Cidade (Cidade_PK, Cidade) VALUES
(1, 'Vitória'),
(2, 'Vila Velha'),
(3, 'Cariacica'),
(4, 'Serra');

-- Preenchendo a tabela USUARIO
INSERT INTO USUARIO (Login, Senha, ID, Nome, Telefone) VALUES
  ('usuario1', 'senha1', 1, 'João Silva', 27999990001),
  ('usuario2', 'senha2', 2, 'Maria Santos', 27999990002),
  ('usuario3', 'senha3', 3, 'Pedro Oliveira', 27999990003),
  ('usuario4', 'senha4', 4, 'Ana Souza', 27999990004);

-- Preenchendo a tabela Bairro
INSERT INTO Bairro (Bairro_PK, Bairro) VALUES
(1, 'Centro'),
(2, 'Jardim Camburi'),
(3, 'Praia do Canto'),
(4, 'São Diogo');

-- Preenchendo a tabela ENDERECO
INSERT INTO ENDERECO (CEP, FK_Tipo_logradouro_Tipo_logradouro_PK, Logradouro, FK_Estado_Estado_PK, FK_Cidade_Cidade_PK, FK_Bairro_Bairro_PK, Numero, ID, Longitude, Latitude) VALUES
  ('29000-000', 1, 'Rua A', 1, 1, 1, 100, 1, 123456, 654321),
  ('29100-000', 2, 'Avenida B', 1, 2, 2, 200, 2, 987654, 456789),
  ('29010-000', 3, 'Travessa C', 1, 3, 3, 300, 3, 567890, 321098),
  ('29110-000', 4, 'Alameda D', 1, 4, 4, 400, 4, 908765, 789012);

-- Preenchendo a tabela INSTITUICAO
INSERT INTO INSTITUICAO (CNPJ, Logo, Telefone, FK_USUARIO_ID, FK_ENDERECO_ID) VALUES
  ('11111111111111', 'logo1.jpg', 27333330001, 1, 1),
  ('22222222222222', 'logo2.jpg', 27333330002, 2, 2),
  ('33333333333333', 'logo3.jpg', 27333330003, 3, 3),
  ('44444444444444', 'logo4.jpg', 27333330004, 4, 4);

-- Preenchendo a tabela PESSOA
INSERT INTO PESSOA (CPF, DataNascimento, FK_USUARIO_ID) VALUES
  ('111.111.111-11', '1990-01-01', 1),
  ('222.222.222-22', '1985-05-10', 2),
  ('333.333.333-33', '1992-03-15', 3),
  ('444.444.444-44', '1988-11-20', 4);

-- Preenchendo a tabela PONTO_COLETA
INSERT INTO PONTO_COLETA (Nome, ID, Imagem, Nota, FK_INSTITUICAO_FK_USUARIO_ID, FK_ENDERECO_ID) VALUES
  ('Ponto de Coleta 1', 1, 'ponto1.jpg', 4.2, 1, 3),
  ('Ponto de Coleta 2', 2, 'ponto2.jpg', 3.9, 2, 4),
  ('Ponto de Coleta 3', 3, 'ponto3.jpg', 4.1, 3, 1),
  ('Ponto de Coleta 4', 4, 'ponto4.jpg', 3.7, 4, 2);

-- Preenchendo a tabela MATERIAIS_RECICLADOS
INSERT INTO MATERIAIS_RECICLADOS (id, descricao) VALUES
  (1, 'Papel'),
  (2, 'Plástico'),
  (3, 'Vidro'),
  (4, 'Metal');

-- Preenchendo a tabela AVALIACAO
INSERT INTO AVALIACAO (id, descricao) VALUES
  (1, 'Ótimo atendimento'),
  (2, 'Local com boas condições'),
  (3, 'Atendimento demorado'),
  (4, 'Falta de organização');

-- Preenchendo a tabela _Cadastra
INSERT INTO _Cadastra (fk_USUARIO_ID, fk_PONTO_COLETA_ID, qtd_validacao, data) VALUES
  (1, 1, 5, '2023-05-01 10:00:00'),
  (2, 2, 3, '2023-05-02 14:30:00'),
  (3, 3, 2, '2023-05-03 09:15:00'),
  (4, 4, 4, '2023-05-04 16:45:00');

-- Preenchendo a tabela Leva
INSERT INTO Leva (fk_MATERIAIS_RECICLADOS_id, fk_PESSOA_FK_USUARIO_ID) VALUES
  (1, 1),
  (2, 2),
  (3, 3),
  (4, 4);

-- Preenchendo a tabela Recicla
INSERT INTO Recicla (fk_MATERIAIS_RECICLADOS_id, fk_PONTO_COLETA_ID) VALUES
  (1, 1),
  (2, 2),
  (3, 3),
  (4, 4);

-- Preenchendo a tabela Envia_PESSOA_MATERIAIS_RECICLADOS_PONTO_COLETA
INSERT INTO Envia_PESSOA_MATERIAIS_RECICLADOS_PONTO_COLETA (fk_PESSOA_FK_USUARIO_ID, fk_MATERIAIS_RECICLADOS_id, fk_PONTO_COLETA_ID) VALUES
  (1, 1, 1),
  (2, 2, 2),
  (3, 3, 3),
  (4, 4, 4);

-- Preenchendo a tabela Realiza_PESSOA_AVALIACAO_PONTO_COLETA
INSERT INTO Realiza_PESSOA_AVALIACAO_PONTO_COLETA (fk_PESSOA_FK_USUARIO_ID, fk_AVALIACAO_id, fk_PONTO_COLETA_ID) VALUES
  (1, 1, 1),
  (2, 2, 2),
  (3, 3, 3),
  (4, 4, 4);

-- Preenchendo a tabela Valida
INSERT INTO Valida (fk_USUARIO_ID, data, resposta) VALUES
  (1, '2023-05-01 10:00:00', 1),
  (2, '2023-05-02 14:30:00', 0),
  (3, '2023-05-03 09:15:00', 1),
  (4, '2023-05-04 16:45:00', 0);

 
 <br> + insert para dados a serem inseridos)
        b) Criar um novo banco de dados para testar a restauracao 
        (em caso de falha na restauração o grupo não pontuará neste quesito)
        c) formato .SQL

#### 12 PRINCIPAIS CONSULTAS DO SISTEMA 
 Inserir as principais consultas (relativas aos 5 principais relatórios) definidas previamente no iten 3.1 deste template.
 <br>
  a) Você deve apresentar as consultas em formato SQL para cad um dos relatórios.
 <br>
  b) Além da consulta deve ser apresentada uma imagem com o resultado obtido para cada consulta.<br>

 ### 13 Gráficos, relatórios, integração com Linguagem de programação e outras solicitações.<br>
     OBS: Observe as instruções relacionadas a cada uma das atividades abaixo.<br>
 #### 13.1	Integração com Linguagem de programação; <br>
 #### 13.2	Desenvolvimento de gráficos/relatórios pertinentes, juntamente com demais <br>
 #### solicitações feitas pelo professor. <br>
 
 ### 14 Slides e Apresentação em vídeo. <br>
     OBS: Observe as instruções relacionadas a cada uma das atividades abaixo.<br>
 #### 14.1 Slides; <br>
 #### 14.2 Apresentação em vídeo <br>

    
##### About Formatting
    https://help.github.com/articles/about-writing-and-formatting-on-github/
    
##### Basic Formatting in Git
    
    https://help.github.com/articles/basic-writing-and-formatting-syntax/#referencing-issues-and-pull-requests
   
    
##### Working with advanced formatting
    https://help.github.com/articles/working-with-advanced-formatting/

#### Mastering Markdown
    https://guides.github.com/features/mastering-markdown/

### OBSERVAÇÕES IMPORTANTES

#### Todos os arquivos que fazem parte do projeto (Imagens, pdfs, arquivos fonte, etc..), devem estar presentes no GIT. Os arquivos do projeto vigente não devem ser armazenados em quaisquer outras plataformas.
1. Caso existam arquivos com conteúdos sigilosos, comunicar o professor que definirá em conjunto com o grupo a melhor forma de armazenamento do arquivo.

#### Todos os grupos deverão fazer Fork deste repositório e dar permissões administrativas ao usuário deste GIT, para acompanhamento do trabalho.

#### Os usuários criados no GIT devem possuir o nome de identificação do aluno (não serão aceitos nomes como Eu123, meuprojeto, pro456, etc). Em caso de dúvida comunicar o professor.


Link para BrModelo:<br>
http://sis4.com/brModelo/brModelo/download.html
<br>


Link para curso de GIT<br>
![https://www.youtube.com/curso_git](https://www.youtube.com/playlist?list=PLo7sFyCeiGUdIyEmHdfbuD2eR4XPDqnN2?raw=true "Title")
